...
public class TableModelExpired extends AbstractTableModel {
	...
	private static final Object[] header = { "Name", "Sektion", "Kategorie", "Ausstellung/Leihgabe"};
	private Object rowData[][];

	public TableModelExpired(ArrayList<Exhibit> data) {
		...
		for (int i = 0; i < data.size(); i++) {
			...
			Long outId = exhibit.getCurrentOutsourced().getId();
			try {
				Outsourced out = Access.getOutsourcedByID(outId);
				outsourced = out.getName();
			} catch (OutsourcedNotFoundException e1) {
				InformationPanel.getInstance().setText(e1.getMessage());
				e1.printStackTrace();
			}

			rowData[i] = new Object[] { exhibit.getName(), section, category,
					outsourced, exhibit.getId() };
		}
